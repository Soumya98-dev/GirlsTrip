...
struct OnboardingView: View {
    ...
    @State private var showingPermissionsPriming = false
    @State private var showingFullNameEntry = false
    @State private var showingCameraPermissionRequest = false // New state variable
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingFullNameEntry) {
            FullNameEntryView { firstName, lastName in
                print("Next tapped with First Name: \(firstName), Last Name: \(lastName)")
                self.userFirstName = firstName // Store if needed
                self.userLastName = lastName   // Store if needed
                self.showingFullNameEntry = false
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    self.showingCameraPermissionRequest = true // Show next screen
                }
            }
        }
        .sheet(isPresented: $showingCameraPermissionRequest) {
            CameraPermissionRequestView(
                onYesTapped: {
                    print("User tapped YES on our custom camera permission screen.")
                ...
                onNoTapped: {
                    print("User tapped NO on our custom camera permission screen.")
                    self.showingCameraPermissionRequest = false
                    self.hasCompletedOnboarding = true
                }
            )
